...
const Contact: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    alert("Message sent! Your inquiry has been successfully forwarded to info@tedxkuleuven.com. Our team will get back to you as soon as possible.");
  };
          ...
          <div className="flex flex-col gap-12">
            ...
            <div className="flex flex-col gap-6 text-left">
              ...
              <div className="flex flex-wrap gap-4">
                <a href={socialLinks.linkedin} className="flex items-center justify-center size-12 bg-white/5 hover:bg-white/10 border border-white/10 transition-all rounded-full group">
                  <span className="material-symbols-outlined text-white/60 group-hover:text-primary transition-colors">share</span>
                </a>
                <a href={socialLinks.instagram} className="flex items-center justify-center size-12 bg-white/5 hover:bg-white/10 border border-white/10 transition-all rounded-full group">
                  <span className="material-symbols-outlined text-white/60 group-hover:text-primary transition-colors">public</span>
                </a>
              </div>
            </div>
          </div>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="flex flex-col gap-3 text-left">
                    ...
                    <input required className="bg-white/[0.03] border border-white/10 rounded-xl p-4 text-white focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary transition-all" placeholder="e.g. Jane Doe" type="text" />
                  </div>
                  <div className="flex flex-col gap-3 text-left">
                    <label className="text-white/40 text-xs font-bold uppercase tracking-widest ml-1">Email Address</label>
                    <input required className="bg-white/[0.03] border border-white/10 rounded-xl p-4 text-white focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary transition-all" placeholder="jane@example.com" type="email" />
                  </div>
                </div>
                <div className="flex flex-col gap-3 text-left">
                  <label className="text-white/40 text-xs font-bold uppercase tracking-widest ml-1">Message</label>
                  <textarea required className="bg-white/[0.03] border border-white/10 rounded-xl p-4 text-white focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary transition-all resize-none" placeholder="Tell us more about your ideas..." rows={6}></textarea>
                </div>
